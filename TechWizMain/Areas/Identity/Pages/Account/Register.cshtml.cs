...
namespace TechWizMain.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserEmailStore<UserManager> _emailStore;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var userUpdate = new IdentityResult();
                var user = CreateUser();
                var userFind = new UserManager();
                ...
                if (result.Succeeded || userUpdate.Succeeded)
                {
                    ...
                    {
                        userId = await _userManager.GetUserIdAsync(userFind);
                        code = await _userManager.GenerateEmailConfirmationTokenAsync(userFind);
                    }
                    else
                    {
                ...
                foreach (var error in result.Errors)
                {
                    ...
                    return Content("Error:" + error.Description);
                }
            }
        ...
        private UserManager CreateUser()
        {
            try
            {
                return Activator.CreateInstance<UserManager>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(UserManager)}'. " +
                    $"Ensure that '{nameof(UserManager)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<UserManager> GetEmailStore()
        {
